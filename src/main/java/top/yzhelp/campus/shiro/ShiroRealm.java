        ...
        AuthorizingRealm realm = new AuthorizingRealm() {
            ...
            protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken)
                ...
                if (!jwtUtil.verify(jwtToken)) {
                    ShiroAccount account =
                        new ShiroAccount("visitor", "visitor", Collections.singletonList(JwtUtil.VISITOR));
                    return new SimpleAuthenticationInfo(account, jwtToken, getName());
                    // throw new AuthenticationException(String.valueOf(WebResultCode.TOKEN_ERROR));
                }
                // 角色信息
