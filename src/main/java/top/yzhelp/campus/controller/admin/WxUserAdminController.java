...
public class WxUserAdminController {
    ...
    public ResponseEntity<Message<?>> all() {
        List<CrgWxUser> list = this.wxUserService.list(new LambdaQueryWrapper<CrgWxUser>().orderByDesc(CrgWxUser::getRealName));
        list.forEach(wxUser -> {
    ...
    @ApiOperation("更新信息")
    @PostMapping
    public ResponseEntity<Message<?>> update(CrgWxUser user) {
